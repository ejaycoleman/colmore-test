...
import { timeSeriesDaily, currentQuote, timeSeriesWeekly, getIndicators } from '../services/SymbolSearch'
...
export default function SymbolInformation ({ symbol }) {
  ...
  useEffect(async () => {
    if (symbol) {
      ...
      const indicatorResult = await getIndicators({ apiKey: apiKeyContext.apiKey, symbol: symbol['1. symbol'] })
      indicatorResult && setIndicatorInfo(indicatorResult)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyItems: 'center', justifyContent: 'center', alignItems: 'center' }}>
      ...
      <h2>Indicator info (SMA)</h2>
      <div style={{ height: 400, width: '100%' }}>
        {indicatorInfo.length !== 0 && <DataGrid disableColumnResize={false} rows={[...indicatorInfo].slice(1)} columns={generateDataGridColumns(indicatorInfo[0])} pageSize={5} />}
      </div>
    </div>
  )
}
